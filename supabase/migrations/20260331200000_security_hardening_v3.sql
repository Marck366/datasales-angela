...
DROP POLICY IF EXISTS "Companies insertable by elevated roles" ON public.companies;

CREATE POLICY "Companies insertable by elevated roles"
  ON public.companies FOR INSERT TO authenticated
  WITH CHECK (public.has_elevated_role(auth.uid()));

-- SEC-007: Blindaje de Eventos
DROP POLICY IF EXISTS "Events updatable by authenticated" ON public.events;
DROP POLICY IF EXISTS "Events updatable by elevated roles" ON public.events;
